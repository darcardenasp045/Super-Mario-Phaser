...
function createPlayer() {
    // Draw player
    player = this.physics.add.sprite(startOffset, screenHeight - platformHeight - 1300, 'mario').setOrigin(1).setBounce(0)  // Sube 70 píxeles (ajusta a 50-100 según pruebes)
        .setCollideWorldBounds(true).setScale(.3);
    player.depth = 3;
...
function updatePlayer(delta) {
    ...
    if (player.y > screenHeight + 50 || timeLeft <= 0) {
        gameOver = true;
        gameOverFunc.call(this);
        return;
    }
